...
1. BAN: 알고리즘 밴 제출
2. PICK: 알고리즘 픽 제출
3. SHOP: 아이템/스펠 선택 → 구매 확정(1회 요청)
4. 룸 채팅 송신

### 6.4 단계 전환 규칙
- BAN 단계 10초 종료 후 서버가 최종 BAN 알고리즘을 산출하고 브로드캐스트한다.
- 브로드캐스트 이후 PICK 단계로 전환되며, 이 시점부터 PICK 10초 카운트가 시작된다.
- PICK 단계에서 최종 BAN 알고리즘은 선택 불가 처리된다(별도 알림 UI 없음).
- PICK 단계 10초 종료 후 서버가 최종 PICK 알고리즘을 산출하고 브로드캐스트한다.
- 브로드캐스트 이후 SHOP 단계로 전환되며, 이 시점부터 SHOP 10초 카운트가 시작된다.
...
### 8.2 주요 구성
- 결과 상태 (WIN/LOSE/DRAW)
- 점수/코인/경험치 변화량 (점수/코인은 RANKED만, 경험치는 NORMAL/RANKED 모두)
- MAIN 이동 버튼
- MY_PAGE 이동 버튼
