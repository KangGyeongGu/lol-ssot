...
- 방장은 다음 기능을 수행할 수 있다.
  ...
  - 플레이어 강퇴
  - 방장 위임(대기실)
- 방장 위임은 다음 조건을 만족해야 한다.
  - 대상은 방에 존재하는 플레이어
  - 상태가 READY 또는 UNREADY
  - 강퇴/이탈 상태가 아님
  - 본인에게 위임 불가
- 방장 위임 성공 시 대상은 READY 상태로 전환되며, 대기실 UI에서 방장 표시가 갱신되어야 한다.
### 3.3 강퇴
...
- 점수(score)는 변동되지 않는다.
- 코인 보상은 지급되지 않는다.
- 경험치 보상은 지급된다. (순위/결과 기반, 상세 규칙은 ECONOMY 참고)
### 4.2 RANKED 게임
- 밴/픽/구매 단계를 거친 후 인게임을 시작한다.
- 아이템 및 스펠 기능이 존재한다.
- 게임 결과에 따라 점수가 변동된다.
- 게임 결과에 따라 코인/경험치 보상이 지급된다.
- BAN 단계 종료(10초) 후 서버가 최종 BAN 알고리즘을 산출하고, 그 결과가 브로드캐스트된 시점에 PICK 단계로 전환된다.
- PICK 단계 종료(10초) 후 서버가 최종 PICK 알고리즘을 산출하고, 그 결과가 브로드캐스트된 시점에 SHOP 단계로 전환된다.
- PICK 단계에서는 최종 BAN 알고리즘이 선택 불가 처리되어야 한다(별도 알림 UI 없음).
---
## 5. 게임 진행 페이지 요구사항
...
- 아이템: 최대 3개
- 스펠: 최대 2개
- 아이템은 종류별 최대 5개까지 구매 가능하다.
- 스펠은 종류별 최대 2개까지 구매 가능하다.
- 구매는 보유 코인 수 내에서만 가능하다.
- SHOP 단계 구매는 아이템/스펠을 일괄 선택 후 단일 요청으로 제출해야 한다.
- 동일 유저의 SHOP 구매 요청은 1회만 허용한다.
### 6.2 사용 방식
- 아이템은 상대 플레이어에게 사용된다.
- 스펠은 자기 자신에게 적용된다.
---
