...
- 방장이 대기실을 떠날 경우,
  ...
  - DISCONNECTED 상태 플레이어는 위임 대상에서 제외한다.
### 2.3 방장 수동 위임
- 방장은 대기실에서 다른 플레이어에게 방장을 위임할 수 있다.
- 위임 대상 조건:
  - 방에 존재하는 플레이어
  - 상태가 READY 또는 UNREADY
  - 강퇴/이탈 상태가 아님
  - 본인에게 위임 불가
- 위임 성공 시:
  - 위임받은 플레이어는 즉시 READY 상태로 전환된다.
  - 대기실 UI에서 방장 표시가 갱신된다.
  - 기존 방장은 일반 플레이어로 전환되며 READY/UNREADY 토글이 가능하다.
### 2.4 게임 시작 조건
...
- 선택은 제한 시간 내에만 유효하다.
### 4.2 BAN 최종 알고리즘 결정
- BAN 단계 종료(10초) 시 서버가 최종 BAN 알고리즘 1개를 산출한다.
- BAN 투표는 플레이어 간 중복이 허용된다.
- 후보 집합:
  - 투표된 알고리즘 목록(중복 제거)
  - 단, 투표가 0건이면 전체 알고리즘 풀
- 가중치:
  - `weight = voteCount` (해당 알고리즘 투표 수)
- 최종 BAN 산출 후 브로드캐스트가 수행되며, 이후 PICK 단계로 전환된다.
### 4.3 PICK 최종 알고리즘 결정
- PICK 단계 종료(10초) 시 서버가 최종 PICK 알고리즘 1개를 산출한다.
- PICK 투표는 플레이어 간 중복이 허용된다.
- 후보 집합:
  - 전체 알고리즘 풀에서 최종 BAN 알고리즘을 제외한 모든 알고리즘
- 가중치:
  - `weight = baseWeight + voteCount * voteWeight`
  - `baseWeight = 1`, `voteWeight = 2`
- 최종 PICK 산출 후 브로드캐스트가 수행되며, 이후 SHOP 단계로 전환된다.
---
## 5. 구매 규칙 (RANKED)
### 5.1 구매 수량 제한
- 아이템 종류: 최대 3개
...
- 서버는 구매 요청 시 다음을 검증한다.
  - 보유 코인이 충분한지
  - 구매 수량/종류 제한을 초과하지 않았는지
- 유효하지 않은 구매는 실패 처리된다.
### 5.3 구매 요청 방식
- SHOP 단계 구매는 **단일 요청**으로 아이템/스펠을 함께 제출한다.
- 동일 유저의 SHOP 구매 요청은 **1회만 허용**한다.
- 요청 내 중복 itemId/spellId는 허용되지 않는다.
- quantity는 1 이상이어야 한다.
- 검증 실패 시 전체 요청은 실패 처리된다(원자적 처리).
---
## 6. 인게임 규칙
### 6.1 플레이 제한 시간
- 최대 플레이 시간은 1시간이다.
